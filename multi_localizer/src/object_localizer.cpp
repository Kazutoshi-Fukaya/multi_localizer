...
double multi_localizer::ObjectLocalizer::get_weight(geometry_msgs::PoseStamped& pose)
{
    ...
    if(USE_OPS_MSG_){
        for(const auto & op : ops_.object_position){
            ...
            Object nearest_object = database_->get_highly_similar_object(op.Class,tmp_x,tmp_y,sim);
...
void multi_localizer::ObjectLocalizer::publish_objects_msg()
{
    ...
    data.pose.weight = weight;
    data.pose.x = estimated_pose_.pose.position.x;
    data.pose.y = estimated_pose_.pose.position.y;
    data.pose.theta = tf2::getYaw(estimated_pose_.pose.orientation);
    for(const auto &op: ops_.object_position){
        multi_localizer_msgs::ObjectData od;
        double distance = std::sqrt(op.x*op.x + op.z*op.z);
        double angle = std::atan2(op.z,op.x) - 0.5*M_PI;

        od.name = op.Class;
        od.time = (now_time - start_time_).toSec();
        od.credibility = weight;
        od.x = data.pose.x + distance*std::cos(data.pose.theta + angle);
        od.y = data.pose.y + distance*std::sin(data.pose.theta + angle);
        data.data.emplace_back(od);
    }
